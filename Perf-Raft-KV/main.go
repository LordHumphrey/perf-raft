...
import (
	...
	httpd "github.com/LordHumphrey/Perf-Raft-KV/http"
	"github.com/LordHumphrey/Perf-Raft-KV/store"
)
...
func main() {
	...
	// 启动成功！
	log.Printf("Perf-Raft-KV 启动成功，监听 http://%s", httpAddr)

	// 等待中断信号
	terminate := make(chan os.Signal, 1)
	signal.Notify(terminate, os.Interrupt)
	<-terminate
	log.Println("Perf-Raft-KV 退出")
}
